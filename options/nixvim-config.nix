...

{
  programs.nixvim = {
	...
	vim.api.nvim_set_keymap('n', '<leader>cf', '<cmd>lua RunInFloatingTerminal("cargo run")<CR>', {noremap = true, silent = true})
	...
	function RunInFloatingTerminal(cmd)
	  ...
	  vim.api.nvim_command('term ' .. cmd) -- run your command in the terminal
